    ...
    function batchUpload(config) {
        ...
        batchUploader.on('uploadSuccess', function(file, json) {
            ...
            if (json.code == 200) {
                ...
                if(config.data.j == undefined){
                    config.data.j = 0;
                }

                $('#' + file.id).append($('<input type="hidden" name="'+config.data.name+'['+ config.data.j +']" value="'+json.url+'">'))
